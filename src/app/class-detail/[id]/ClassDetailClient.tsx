...
import { enrollInClass, checkEnrollment, closeClass } from '@/actions/class'
...
export default function ClassDetailClient({ classData, user, relatedClasses, classId }: ClassDetailClientProps) {
  ...
  // 페이지 로드 시 수강 신청 상태 확인
  useEffect(() => {
    async function checkUserEnrollment() {
      if (!user) return
      
      const { isEnrolled } = await checkEnrollment(user.id, classId)
  ...
  const handleEnroll = async () => {
    ...
    const { error } = await enrollInClass(user.id, classId)
    ...
    // 수강 인원 업데이트
    const newTotal = (updatedClassData.students_total || 0) + 1
    setUpdatedClassData({
      ...updatedClassData,
  ...
  const handleCloseClass = async () => {
    ...
    const { error } = await closeClass(classId)
  ...
  const formattedStartDate = new Date(updatedClassData.start_date).toLocaleString('ko-KR', {
    year: 'numeric',
    month: '2-digit',
    day: '2-digit',
    hour: '2-digit',
  ...
  const formattedEndDate = new Date(updatedClassData.end_date).toLocaleString('ko-KR', {
    year: 'numeric',
    month: '2-digit',
    day: '2-digit',
    hour: '2-digit',
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-2/3">
          ...
          <div className="relative bg-blue-900 text-white p-8 rounded-lg mb-8">
            ...
            <div className="md:flex items-center mt-6">
              <div className="md:w-2/3">
                ...
                <h1 className="text-3xl font-bold mb-4">{updatedClassData.title}</h1>
                <p className="mb-4">
                  AI 홈페이지 수익화 마스터 클래스
                </p>
              </div>
              ...
              <div className="md:w-1/3 flex justify-center">
                {updatedClassData.thumbnail_img ? (
                  <Image 
                    src={updatedClassData.thumbnail_img} 
                    alt={updatedClassData.lecturer} 
                    width={200} 
                    height={250}
          ...
          <div className="mb-8">
            ...
            {updatedClassData.detail_img ? (
              <div className="mb-4">
                <Image 
                  src={updatedClassData.detail_img} 
                  alt="클래스 소개 이미지" 
                  width={800} 
                  height={400}
            ...
            <div className="prose max-w-none">
              <p>{updatedClassData.detail_text || '클래스 상세 설명이 준비 중입니다.'}</p>
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6 sticky top-20">
            ...
            <div className="mb-6">
              <span className="text-2xl font-bold">
                {updatedClassData.price === 0 ? '무료' : `${updatedClassData.price.toLocaleString()}원`}
              </span>
            </div>
            ...
            <div className="space-y-4 text-sm text-gray-600">
              ...
              <div className="flex items-center">
                ...
                <span>방송일: {new Date(updatedClassData.start_date).toLocaleDateString('ko-KR')}</span>
              </div>
              <div className="flex items-center">
                <svg 
                  xmlns="http://www.w3.org/2000/svg" 
                ...
                <span>강사: {updatedClassData.lecturer}</span>
              </div>
              <div className="flex items-center">
                <svg 
                  xmlns="http://www.w3.org/2000/svg" 
                ...
                <span>수강생: {updatedClassData.students_total || 0} / {updatedClassData.students_max || '무제한'}</span>
              </div>
            </div>
              ...
              {isAdmin ? (
                <>
                  ...
                  <div className="space-y-2">
                    <button 
                      className="block w-full py-2 px-4 rounded-md bg-gray-100 text-gray-700 text-sm text-center hover:bg-gray-200"
                      onClick={() => router.push(`/admin/update-class/${classId}`)}
                    >
                      클래스 수정
                    </button>
                    <button 
                      className="block w-full py-2 px-4 rounded-md bg-red-100 text-red-600 text-sm text-center hover:bg-red-200"
                      onClick={handleCloseClass}
                    >
                      클래스 마감
                    </button>
